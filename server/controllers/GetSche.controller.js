...
export const findCourse = async (req, res) => {
    try {
        ...
        maMonHoc = maMonHoc.replace(/\s+/g, "");
        const course = await prisma.examPeriod.findFirst({ where: { name: { contains: maMonHoc, mode: "insensitive" } } });
...
export const getExamSche = async (req, res) => {
    try {
        ...
        const $ = cheerio.load(response.data);
        const url = $('a.txt_l4[href$=".xlsx"]').attr("href");
    ...
    try {
        ...
        const results = await prisma.examStudent.findMany({
            where: {
                ...
                examSchedule: {
                    courseCode: { contains: maMonHoc, mode: "insensitive" },
                },
            },
            include: {
                examSchedule: {
